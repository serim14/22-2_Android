...
public class MainActivity extends AppCompatActivity {
    ...
    public void makeRequest() {
        ...
        StringRequest request = new StringRequest(  // 문자열로 받음, 생성자, 4개 파라미터
                ...
                url,
                new Response.Listener<String>() {   // 제너릭 인터페이스
                   @Override
                ...
                new Response.ErrorListener() {  // 실패 콜백 함수(서버에서 결과 답을 주게 되면 해당하는 메서드 호출)
                    @Override
    ...
    public void parseHtml(String html) {    // 파라미터명 html
        ...
        Elements titleElements = doc.select("div.title1 a h6");
        Elements urlElements = doc.select("div.title1 a");
